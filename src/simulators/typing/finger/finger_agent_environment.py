...
from ..utilities.utils import parse_transition_index, distance, WHo_mt
...
class FingerAgentEnv(AgentEnv):
    def __init__(self, layout_config, agent_params, finger, train):
        ...
        self.observation_space = gym.spaces.Box(low=0.0, high=1.0, shape=(
            len(self.device.keys) +  # one-hot encoding of target
            self.n_finger_locations +  # one-hot encoding of finger location
            1 +  # sat desired
            1,  # entropy.
        )) ### obs_dim: 75 (English layout)
        ...
        self.transition_model_path = os.path.join(Path(__file__).parent.parent, "models")
    ...
    def initialise_transition(self):
        ...
        if os.path.exists(os.path.join(self.transition_model_path, self.transition_file)):
            self.logger.info("Transition model exists. Loading existing model.")
            self.transition_model = pd.read_csv(os.path.join(self.transition_model_path, self.transition_file), index_col=0)
    ...
    def create_transition_model(self):
        ...
        self.transition_model.to_csv(os.path.join(self.transition_model_path, self.transition_file))
        self.logger.info("Saved the transition table at {%s}" % os.path.join(self.transition_model_path, self.transition_file))
    ...
    def step(self, action, sigma=None):
        ...
        :param sigma: precision of finger movement given by supervisor
        :return: tuple <next state, reward, done, info>

        ### One modification from CHI'21 paper:
        ### finger movement's sigma (sat_true) is determined by supervisor agent's action
        """
        if sigma is None:
            sigma = np.random.choice(self.sat_true_list)
        ...
        self.logger.debug(
            "Taking {%s} move with (x.diff, y.diff) : {%s},  sat {%.2f} and desired sat {%.2f}" % (
        ...
        # take action.
        _, movement_time = self.move_finger(action, sigma)
        ...
        # Added: Set max ep length (for train)
        self.ep_step += 1
    ...
    def move_finger(self, action, sigma):
        ...
        movement_time = WHo_mt(
            self.dist,
            sigma,
            alpha=self.free_params["who_alpha"],
            k=self.free_params["who_k"],
        )
        ...
        new_loc, is_error = self.update_sensor_position(action, sigma)
    ...
    def update_finger_loc_prob(self, action):
        ...
        s = list(product(list(range(self.n_finger_locations)), [0.266], [action]))
        s = list(map(str, s))
